		...
		if(isset($_POST['submitPo']))
		{
			...
			extract($_POST);
				...
				if($leadStatus=="PO collected")
				{
					...
					mysql_query("INSERT INTO `expected_collections`(`expectedId`, `reportId`, `amount`, `expectedDate`, `dateTime`) VALUES ('',".$reportId.",'".$amount."','".date('Y-m-d')."','".$dateTime."')");

//get the customer id 

$cusqry = mysql_query("select company from daily_reports where reportId=".$reportId);

$customerid = mysql_fetch_object($cusqry);

					
					mysql_query("INSERT INTO `expected_sales`(`expectedSaleId`, `customerId`, `product`, `quantity`, `expectedValue`, `expectedDate`, `dateTime`, `employeeId`) VALUES ('',".$customerid->company.",'','','".$amount."','".date('Y-m-d')."','".date('Y-m-d H:i:s')."',".$_SESSION['id'].")");
			...
			mysql_query("update daily_reports set leadStatus = '". $_POST['leadStatus'] ."', po = '1', poNo = '". $_POST['po'] ."', poDateTime = NOW() where reportId = '". $_POST['reportId'] ."'");
			header("location: add_po.php?rid=".$_GET['rid']."&add=1");
			
			}*/
				// if orders lost
				if(strcmp($_POST['leadStatus'],'Order Lost')==0)
				{
				   mysql_query("update daily_reports set remarks='".$_POST['remarks']."', reasonChange = '".$_POST['reasonChange']."', toWhom = '".$_POST['toWhom']."', leadStatus = '". $_POST['leadStatus'] ."', lostDateTime = NOW() where reportId = '". $_POST['reportId'] ."'");	
				   
			//	   mysql_query("update invoices set invoiceStatus = '". $_POST['leadStatus'] ."', lostDateTime = NOW() where invoiceId = '". $_POST['invoiceId'] ."'");	
				   
				}
				
				else if(strcmp($_POST['leadStatus'],'Offer submitted','Offer to be resubmitted','Offer resubmitted','PO to be collected')==0)
				{
				   mysql_query("update daily_reports set remarks='".$_POST['remarks']."', leadStatus = '". $_POST['leadStatus'] ."' where reportId = '". $_POST['reportId'] ."'");	
				   
			//	   mysql_query("update invoices set invoiceStatus = '". $_POST['leadStatus'] ."', lostDateTime = NOW() where invoiceId = '". $_POST['invoiceId'] ."'");	
				   
				}
				else if(strcmp($_POST['leadStatus'],'PO collected')==0)
				{
			...
			header("location: add_po.php?rid=".$_GET['rid']."&status=1");	
			}
		
 // select statement for daily reports records 
$proposals = mysql_query("select daily_reports.reportId, daily_reports.offerNo, daily_reports.delCommDate, date_format(daily_reports.offerDate,'%d-%m-%Y') as offerDate, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.enquiryNumber, daily_reports.delAcceptanceDate, daily_reports.paymentTerms, daily_reports.reportDate, daily_reports.currentRevisionId, daily_reports.company, daily_reports.contactPerson, daily_reports.designation, daily_reports.phone, daily_reports.email, daily_reports.clientStatus, daily_reports.leadType, daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.po, daily_reports.poNo, daily_reports.poDateTime, daily_reports.paymentType, customers.company, employees.firstName from daily_reports
...
where daily_reports.reportId = '". $_GET['rid'] ."'");
 ...
 $revision = mysql_query("select revisionId, subTotal, discount, duty, dutyPercentage, pf, pfPercentage, total, grandTotal  from daily_reports_revision where reportId = '". $proposal['reportId'] ."' and revision = '". $proposal['currentRevisionId'] ."'");
  ...
  // select statement to get the product records from products table 
 $items = mysql_query("select categories.category, brands.brand, products.productId,pm.ModelNo, products.product, daily_reports_data.id, daily_reports_data.price, daily_reports_data.quantity, daily_reports_data.amount, daily_reports_data.taxSystem, daily_reports_data.taxPercentage, daily_reports_data.taxAmount 
...
{ echo '<div class="alert alert-danger"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Error occured, please try again.</div>'; }
                                      ...
                                      <table id="sample-table-1" class="table table-striped table-bordered table-hover">
											<tbody>
                                                ...
                                                <tr>
                                                   ...
                                                   <td>
													 <?php
													  $delCommDate = explode('-',$proposal['delCommDate']);
													  $delCommDate = $delCommDate[2].'-'.$delCommDate[1].'-'.$delCommDate[0];  	
													  
													  if(strcmp('00-00-0000',$delCommDate)!=0)
												{
													echo $delCommDate;
												}												  
 													  ?>
                                                   ...
                                                   <td>
													 <?php
													  $delAcceptanceDate = explode('-',$proposal['delAcceptanceDate']);
													  $delAcceptanceDate = $delAcceptanceDate[2].'-'.$delAcceptanceDate[1].'-'.$delAcceptanceDate[0];  	
													  
													  if(strcmp('00-00-0000',$delAcceptanceDate)!=0)
												{
													echo $delAcceptanceDate;
												}												  
 													  ?>
												  </td>
												</tr>
                              ...
                              <tr><td colspan="6"></td><td><?php echo $item['taxSystem'].' '.$item['taxPercentage'].'%'; ?></td><td><?php echo $item['taxAmount']; ?></td></tr>
								...
								$i++;	 }
                                    ...
                                    <tr><td colspan="6"></td><td>Duty 12.5%</td><td><?php echo $revision['duty']; ?></td></tr> 
                                
                                    <tr><td colspan="6"></td><td>PF 3%</td><td><?php echo $revision['pf']; ?></td></tr> 
                               
                                    <tr><td colspan="6"></td><td>Grand Total</td><td><?php echo $revision['grandTotal']; ?> <input type="hidden" name="order_grandtotal" value="<?PHP echo $revision['grandTotal']; ?>" /></td></tr> 
...
<option value="Offer submitted" <?php if ($proposal['leadStatus'] == 'Offer submitted') echo ' selected="selected"'; ?>>Offer submitted</option>
<option value="Offer to be resubmitted" <?php if ($proposal['leadStatus'] == 'Offer to be resubmitted') echo ' selected="selected"'; ?>>Offer to be resubmitted</option>
<option value="Offer resubmitted" <?php if ($proposal['leadStatus'] == 'Offer resubmitted') echo ' selected="selected"'; ?>>Offer resubmitted</option>
<option value="PO to be collected" <?php if ($proposal['leadStatus'] == 'PO to be collected') echo ' selected="selected"'; ?>>PO to be collected</option>
<option value="PO collected" <?php if ($proposal['leadStatus'] == 'PO collected') echo ' selected="selected"'; ?>>PO collected</option>
<option value="Order Lost" <?php if ($proposal['leadStatus'] == 'Order Lost') echo ' selected="selected"'; ?>>Order Lost</option>
                                           </select>
                                   ...
                                   <td colspan="6"></td>
                                    ...
                                    <td><textarea class="col-xs-10 col-sm-12" name="remarks"></textarea></td>
                                   </tr>
                                    <tr id="ordLost" style="display:none;"><td colspan="5"></td>
                                     <td>
                                        <label for="reasonChange">Reason for Change</label>
                                     </td>
                                     <td>
                                     <input type="hidden" name="reportId" value="<?php echo $proposal['reportId'];  ?>"  />
                                     <input type="text" name="reasonChange" id="reasonChange" placeholder="Reason For Change" />
                                     </td>
                                     
                                      <td>
                                        <label for="toWhom">To which company we lost</label>
                                     </td>
                                     <td>
                                     <input type="text" name="toWhom" id="toWhom" placeholder="To which company we lost" />
                                     </td></tr>
                                    
                                     <input type="hidden" name="reportId" value="<?php echo $proposal['reportId'];  ?>"  />
                                     <input type="text" name="po" id="po" style="display:none;"  />
...
<![endif]-->
  ...
  function addPo(val)
 {
	 ...
	 if(val==='Order Lost')
	 {
		 ...
		 document.getElementById('ordLost').style.display = 'block';	
	 }
	 else
	 {   
		 document.getElementById('ordLost').style.display = 'none';	 
	 }
