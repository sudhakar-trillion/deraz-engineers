...
$result = mysql_query("select daily_reports.reportId, daily_reports.currentRevisionId, daily_reports.enquiryNumber, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.offerNo, daily_reports.contactPerson, daily_reports.designation, daily_reports.phone, daily_reports.email, daily_reports.clientStatus, daily_reports.leadType, daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.poNo, daily_reports.poValue, daily_reports.poDateTime, daily_reports.paymentType, daily_reports.addedOn, daily_reports.total, customers.company  from daily_reports 
 						...
 						left join enquiries on daily_reports.enquiryNumber = enquiries.enquiryNumber
						  where daily_reports.reportId = '". $_GET['rid'] ."' and daily_reports.addedBy = '". $_SESSION['id'] ."'
						  order by daily_reports.reportId desc");
						  
						 
	$row = mysql_fetch_array($result);
	
$revision = mysql_query("select revisionId, subTotal, discount, duty, dutyPercentage, vat, vatPercentage, pf, pfPercentage, total, grandTotal , revisionDateTime  from daily_reports_revision where reportId = '". $row['reportId'] ."' and revision = '". $row['currentRevisionId'] ."'");
 
  $revision = mysql_fetch_array($revision);
 
 
  // data comes from daily_reports,daily_reports_data,categories,brands,products tables
 $orders = mysql_query("select categories.category, brands.brand, products.productId, products.product, daily_reports_data.id, daily_reports_data.price, daily_reports_data.quantity, daily_reports_data.amount, daily_reports_data.taxSystem, daily_reports_data.taxPercentage, daily_reports_data.taxAmount  
...
{ $alertMsg = '<div class="alert alert-danger"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Error occured, please try again.</div>'; }
  ...
  if(isset($alertMsg)) { echo $alertMsg; }
         							...
         							<tr><td> Po No. </td><td><?php  echo $row['poNo']; ?></td></tr>
											...
											<tr><td> Offer Date. </td><td><?php
															    $date = explode(' ',$revision['revisionDateTime']); 
														     $date = explode('-',$date[0]); 
														     $date = $date[2].'-'.$date[1].'-'.$date[0];
