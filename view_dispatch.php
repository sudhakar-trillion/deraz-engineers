...
/* the relation between invoices and dispatch is invoiceId && the relation between daily_reports and invoices is reportId */

// get the dispatch details and products from dispatch table and dispatch_items table.
$orders = mysql_query("select invoices.invoiceNumber, employees.firstName, invoices.invoiceDateTime, daily_reports.runcard, daily_reports_data.quantity, dispatch.dispatchedId, dispatch.invoiceId, dispatch.dispatchedOn, dispatch_items.remarks, dispatch_items.dispatchedQuantity, products.product, products.code, customers.company from dispatch
...
where invoices.invoiceId = '". $_GET['diid'] ."'");
 
// echo mysql_num_rows($orders); exit;
  
/* 
$revision = mysql_query("select * from daily_reports_revision where reportId = '". $order['reportId'] ."' and revision = '". $order['currentRevisionId'] ."'");

 $revision = mysql_fetch_array($revision);

$items = mysql_query("select categories.category, brands.brand, products.product, products.code, daily_reports_data.price, daily_reports_data.quantity, daily_reports_data.amount from daily_reports_data
  ...
  where daily_reports_data.revisionId = '". $revision['revisionId'] ."' and daily_reports_data.invoice_id = ''");*/
  
  // get products data from categories , brands and daily_reports_data and products table. 
  $items = mysql_query("select categories.category, brands.brand, products.product, products.code, daily_reports_data.price, daily_reports_data.quantity, daily_reports_data.amount, daily_reports_data.taxSystem, daily_reports_data.taxPercentage, daily_reports_data.taxAmount from daily_reports_data
  left join categories on daily_reports_data.categoryId = categories.id
  left join brands on daily_reports_data.brandId = brands.id
  left join products on daily_reports_data.productId = products.productId
  where daily_reports_data.invoice_id = '". $order['invoiceId'] ."'");
