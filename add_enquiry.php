...
/* in this add enquiry form there are multiple tables involved. we are inserting data into enquiries, enquiry_assign, enquiry_products and daily_reports,daily_reports_data tables. */
...
$reportDate = $reportDate[1].'/'.$reportDate[2].'/'.$reportDate[0];
 ...
 else
 {
				 ...
				 if($_POST['companyId']==0)
   {
			...
			$companyName = $customerNumber['company'];*/
/*
...
// inserting data into daily_reports table 
		   mysql_query("insert into daily_reports (`offer`, `currentRevisionId`, `reportDate`, `reportDt`, `enquiryNumber`, `company`, `contactPerson`, `designation`, `phone`, `email`, `clientStatus`, `leadType`, `leadStatus`, `futureDate`, `remarks`, `po`, `paymentType`, `addedOn`, `addedBy`) values ('". $offer ."', '0', '". $date ."', NOW(), '". $_POST['enquiryNumber'] ."', '". $_POST['company'] ."', '". $_POST['contactPerson'] ."', '". $_POST['designation'] ."', '". $_POST['phone'] ."', '". $_POST['email'] ."', '". $_POST['clientStatus'] ."', '". $_POST['leadType'] ."', '". $_POST['leadStatus'] ."', '". $futureDate ."', '". addslashes($_POST['remarks']) ."', '0', '". $_POST['paymentType'] ."', NOW(), '". $_POST['eid'] ."')");		

	 $lastInsertedId = mysql_insert_id();
		  
// inserting data into enquiries table
mysql_query("insert into enquiries (`enqDate`, `enquiryNumber`,`reportId`,`name`, `company`, `companyId`,  `phone`, `email`, `source`, `addedBy`,`dateTime`) values ('".$date."','".$_POST['enquiryNumber']."', '".$lastInsertedId."','". $_POST['contactPerson'] ."', '". $_POST['company'] ."', '". $companyId ."', '". $_POST['phone'] ."', '". $_POST['email'] ."', '". $_POST['source'] ."', '". $_SESSION['id'] ."', NOW())");


			
			$enquiryId = mysql_insert_id();
			
// inserting data into enquiry_assign table		
mysql_query("insert into enquiry_assign (`enquiryId`,`assignedTo`,`assignedBy`,`dateandtime`) values ('". $enquiryId ."', '". $_POST['employee'] ."', '". $_SESSION['id'] ."', NOW())");
			...
			if($lastInsertedId>0)
			{
				if(isset($_POST['po']))
				{
				  ...
				  mysql_query("insert into daily_reports_revision (`reportId`, `revision`, `revisionDateTime`) values ('". $lastInsertedId ."', '0', NOW())");
				...
				$getPrice=mysql_fetch_object($getprice_qry);
					...
					mysql_query("insert into enquiry_products (`enquiryId`, `eaId`, `codeId`,`productId`, `brandId`, `categoryId`, `quantity` ) values ('".$enquiryId."','".$assign_id."', ". $_POST['models'][$i] .",  ". $_POST['products'][$i] .", ". $_POST['brands'][$i] .", ". $_POST['categories'][$i] ."," . $_POST['quantity'][$i] . ")");
  ...
  if(isset($alertMsg)) { echo $alertMsg; }
									...
									<!-- #section:elements.form -->
									 <div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" id="date" name="date" placeholder="Enquiry Date" class="col-xs-10 col-sm-12 date-picker"  />
										</div>
									</div>
