<?php include("includes/header.php"); 
	   ...
	   mysql_query("insert into teams (`teamLeaderId`, `dateTime`) values ('". $_POST['teamLeader'] ."',  NOW())");			
			...
			$leader = mysql_query("select firstName, email from employees where id = '". $_POST['teamLeader'] ."'");
			$leader = mysql_fetch_array($leader);
		    // mail code to team leader
			...
			$message = 'Hi '.$leader['firstName'].',
						You have been assigned as a team leader at Trismart.';
			...
			mail($leader['email'],$subject,$message,$headers);
			  ...
			  mysql_query("insert into team_members (`teamId`, `memberId`, `dateTime`) values ('". $lastId ."', '". $_POST['selectedMembers'][$i] ."', NOW())");
			  
			$member = mysql_query("select firstName, email from employees where id = '". $_POST['teamLeader'] ."'");
			if(mysql_num_rows($member)) {
			$member = mysql_fetch_array($leader);
			// 
			$subject = 'Team Creation';
			...
			mail($member['email'],$subject,$message,$headers);	
			  ...
			  header("location: add_team.php?add=1");	
			}
			else
			{
