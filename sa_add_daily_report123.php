<?php  include("includes/sa_header.php"); 
 ...
 else
 {
	...
	mysql_query("insert into daily_report_authorization (`employeeId`, `authorization`, `reportDate`, `createdOn`, `reportType`) values ('". $_SESSION['id'] ."', 'No', '". $date ."', NOW(), 'daily')");			
		   ...
		   mysql_query("insert into daily_reports (`offer`, `currentRevisionId`, `reportDate`, `company`, `contactPerson`, `designation`, `phone`, `email`, `clientStatus`, `leadType`, `leadStatus`, `futureDate`, `remarks`, `po`, `paymentType`, `addedOn`, `addedBy`) values ('". $offer ."', '0', '". $date ."', '". $_POST['company'] ."', '". $_POST['contactPerson'] ."', '". $_POST['designation'] ."', '". $_POST['phone'] ."', '". $_POST['email'] ."', '". $_POST['clientStatus'] ."', '". $_POST['leadType'] ."', '". $_POST['leadStatus'] ."', '". $futureDate ."', '". addslashes($_POST['remarks']) ."', '0', '". $_POST['paymentType'] ."', NOW(), '". $_SESSION['id'] ."')");			
...
$departments = mysql_query("select customerId, department from customers where addedBy = '". $_SESSION['id'] ."' order by company");
?>
                  ...
                  <input type="text" id="date" name="date" placeholder="Date" class="col-xs-10 col-sm-12 date-picker"  value="<?php echo $reportDate; ?>"  />
                                    ...
                                    <div class="form-group">
                               		...
                               		<div class="form-group" id="department_id">
										...
										<div class="col-sm-9">
											<select id="department" name="department" class="col-xs-10 col-sm-12" onChange="getCokkkDetails(this.value)">
                                            <option value="">Select Department</option>
                                            <?php
											while($department = mysql_fetch_array($departments))
											{	?> 
                                            	<option value="<?php echo $company['customerId'] ?>"><?php echo $company['department'] ?></option><?php  } ?>
                                            </select>
										</div>
									</div>
...
<![endif]-->
 ...
 function getCompanyDetails(cid)
 {
	  ...
	  $.ajax({url: "ajax/getCompanyDetails123.php?cid="+cid, success: function(result){
