 ...
 ?>
  ...
  <div class="row">
      ...
      <?PHP
		...
		$qry_prdct = mysql-query("select drd.price,drd.quantity, prd.product,inv.invoiceNumber from daily_reports_data drd join invoices inv on drd.invoice_id=inv.invoiceId join products prd on prd.productId=drd.productId where inv.invoiceNumber='".$_GET['invoiceId']."' order by drd.id desc limit 1");
