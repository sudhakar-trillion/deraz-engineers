...
$numRecords = mysql_query("select daily_reports.reportId, daily_reports.reportDate, daily_reports.poNo, daily_reports.poDateTime, daily_reports.invoice, daily_reports.invoiceDateTime, customers.company, employees.firstName from daily_reports left join customers on daily_reports.company = customers.customerId
left join employees on daily_reports.addedBy = employees.id where daily_reports.inv = '2' and daily_reports.leadStatus != 'Order Closed' order by daily_reports.reportId desc");
...
// search options 
    $where = 'where ';
   $whereItems[] = "daily_reports.leadStatus!= 'Order Closed'";
...
$proposals = mysql_query("select daily_reports.reportId,  daily_reports.reportDate, daily_reports.poNo, daily_reports.poDateTime, daily_reports.invoice,daily_reports.leadStatus, daily_reports.invoiceDateTime,  customers.company, employees.firstName from daily_reports left join customers on daily_reports.company = customers.customerId left join employees on daily_reports.addedBy = employees.id where daily_reports.inv = '2' and daily_reports.leadStatus!= 'Order Closed' order by daily_reports.reportId desc limit $start, $limit");
...
order by daily_reports_data.reportId desc");
    ...
    ?>			</tbody>
					 ...
					 ?>
                                                        ...
                                                        <td>
                                                        <?php echo $diff = $grandTotal-$collectedAmount; 
...
$rowlist[] = $proposal['leadStatus'];
 ...
 $i++;
  } ?><tr><td colspan="3"></td><td><?php echo $finalTotal; ?></td><td colspan="2"></td><td><?php echo $finalCollected; ?></td><td><?php echo $finalPending; ?></td><td colspan="4"></td></tr> <?php } else {
