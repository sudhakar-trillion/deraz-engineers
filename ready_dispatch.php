...
$query ="select invoices.invoiceNumber, invoices.invoiceId, invoices.total, daily_reports.poNo, invoices.invoiceId, date_format(daily_reports.poDateTime,'%d-%m-%Y') podtm, daily_reports.runcard, invoices.invoiceDateTime, customers.customerId, branches.branch, invoices.proInvoiceNumber, date_format(invoices.proInvoiceDateTime,'%d-%m-%Y') prodate, date_format(daily_reports.rcDate,'%d-%m-%Y') rcDate, employees.firstName, daily_reports.runcard, daily_reports_data.quantity, daily_reports.enquiryNumber,products.product, products.code, customers.company from invoices  join dispatch on dispatch.invoiceId!=invoices.invoiceId  join daily_reports on daily_reports.reportId = invoices.reportId  join customers on daily_reports.company = customers.customerId  join daily_reports_revision as drv on drv.reportId=daily_reports.reportId  join daily_reports_data on daily_reports_data.revisionId=drv.revisionId  join products on daily_reports_data.productId = products.productId  join employees on daily_reports.addedBy = employees.id left join branches on employees.branch = branches.branchId  join categories as cat on cat.id=daily_reports_data.categoryId where daily_reports.reportId=invoices.reportId and dispatch.invoiceId!=invoices.invoiceId and dispatch.dispatchedId = '".$_GET['expDate']."' group by invoices.invoiceNumber order by invoices.invoiceNumber desc limit $start, $limit";
...
$query ="select invoices.invoiceNumber, invoices.invoiceId, invoices.total, daily_reports.poNo, invoices.invoiceId, date_format(daily_reports.poDateTime,'%d-%m-%Y') podtm, daily_reports.runcard, invoices.invoiceDateTime, customers.customerId, branches.branch, invoices.proInvoiceNumber, date_format(invoices.proInvoiceDateTime,'%d-%m-%Y') prodate, date_format(daily_reports.rcDate,'%d-%m-%Y') rcDate, employees.firstName, daily_reports.runcard, daily_reports_data.quantity, daily_reports.enquiryNumber,products.product, products.code, customers.company from invoices  join dispatch on dispatch.invoiceId!=invoices.invoiceId  join daily_reports on daily_reports.reportId = invoices.reportId  join customers on daily_reports.company = customers.customerId  join daily_reports_revision as drv on drv.reportId=daily_reports.reportId  join daily_reports_data on daily_reports_data.revisionId=drv.revisionId  join products on daily_reports_data.productId = products.productId  join employees on daily_reports.addedBy = employees.id left join branches on employees.branch = branches.branchId  join categories as cat on cat.id=daily_reports_data.categoryId where daily_reports.reportId=invoices.reportId and dispatch.invoiceId!=invoices.invoiceId and dispatch.dispatchedId = '".$_GET['dispatchedId']."' group by invoices.invoiceNumber order by invoices.invoiceNumber desc limit $start, $limit";
}

else								
{
$where = 'dispatch.invoiceId!=dispatch.invoiceId';	

	$query = "select invoices.invoiceNumber, invoices.invoiceId, invoices.total, daily_reports.poNo, invoices.invoiceId, date_format(daily_reports.poDateTime,'%d-%m-%Y') podtm, daily_reports.runcard, invoices.invoiceDateTime, customers.customerId, branches.branch, invoices.proInvoiceNumber, date_format(invoices.proInvoiceDateTime,'%d-%m-%Y') prodate, date_format(daily_reports.rcDate,'%d-%m-%Y') rcDate, employees.firstName, daily_reports.runcard, daily_reports_data.quantity, daily_reports.enquiryNumber,products.product, products.code, customers.company from invoices  join dispatch on dispatch.invoiceId!=invoices.invoiceId  join daily_reports on daily_reports.reportId = invoices.reportId  join customers on daily_reports.company = customers.customerId  join daily_reports_revision as drv on drv.reportId=daily_reports.reportId  join daily_reports_data on daily_reports_data.revisionId=drv.revisionId  join products on daily_reports_data.productId = products.productId  join employees on daily_reports.addedBy = employees.id left join branches on employees.branch = branches.branchId  join categories as cat on cat.id=daily_reports_data.categoryId where daily_reports.reportId=invoices.reportId and dispatch.invoiceId!=invoices.invoiceId group by invoices.invoiceNumber order by invoices.invoiceNumber desc limit $start, $limit";
}
...
$qrey = mysql_query("select * from dispatch where invoiceId=".$row['invoiceId']);	
...
{
   window.location = 'ready_dispatch.php?page='+pid;	
}
