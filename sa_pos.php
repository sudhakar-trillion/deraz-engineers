...
$numRecords = mysql_query("select daily_reports.reportId, daily_reports.reportDate, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.contactPerson, daily_reports.enquiryNumber, daily_reports.phone,  daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.poNo, daily_reports.poValue, daily_reports.paymentType, daily_reports.addedOn, customers.company  from daily_reports 
						  ...
						  where daily_reports.po > '0' and daily_reports.addedBy = '".$Emply_ide."'
 ...
 $numRecords = mysql_query("select daily_reports.reportId, daily_reports.reportDate, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.contactPerson, daily_reports.enquiryNumber, daily_reports.phone,  daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.poNo, daily_reports.poValue, daily_reports.paymentType, daily_reports.addedOn, customers.company  from daily_reports 
                          left join customers on daily_reports.company = customers.customerId
  						  left join enquiries on daily_reports.enquiryNumber = enquiries.enquiryNumber
						  where daily_reports.po > '0' and daily_reports.addedBy = '". $_SESSION['id'] ."'
						  GROUP BY daily_reports.reportId desc");
 }
...
// data comes from daily_reports,customers,enquiries tables											
$result = mysql_query("select daily_reports.reportId, daily_reports.offerNo, date_format(daily_reports.offerDate,'%d-%m-%Y') as offerDate,  daily_reports.offerToBeSubmitted, daily_reports.enquiryNumber, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.contactPerson, daily_reports.designation, daily_reports.phone, daily_reports.email, daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.po, daily_reports.poNo, daily_reports.poValue, daily_reports.poDateTime, customers.company, daily_reports.reportType  from daily_reports 
                          left join customers on daily_reports.company = customers.customerId
  						  left join enquiries on daily_reports.enquiryNumber = enquiries.enquiryNumber
						  $where 
						  GROUP BY daily_reports.reportId desc ");
...
<a href="excel/sa_pos.csv" title="Download"><span class="badge badge-success"><i class="ace-icon fa fa-cloud-download bigger-110 icon-only"></i></span></a>
...
{ $alertMsg = '<div class="alert alert-danger"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Error occured, please try again.</div>'; }
  ...
  <?php 
    ...
    $list[] = array('S. No', 'Offer', 'Po Date', 'Po No', 'Company', 'Person', 'Phone', 'Lead Type', 'Lead Status' );
...
$price = mysql_fetch_array($prices);*/
    ...
    if($row['reportType']==1)
	 ...
	 ?><tr style="background:<?php echo $bgcolor; ?>">
                                            ...
                                            <a class="btn btn-success btn-sm" href="sa_view_po.php?rid=<?php echo $row['reportId'] ?>">
