...
left join enquiries on daily_reports.enquiryNumber = enquiries.enquiryNumber
left join branches on employees.branch = branches.branchId
left join daily_reports_revision drv on drv.reportId=daily_reports.reportId
left join daily_reports_data as drd on drd.revisionId=drv.revisionId
left join categories as cat on cat.id=drd.categoryId
left join product_model as pm on pm.ModelId=drd.modelId
$where
 GROUP BY reportId desc");							
...
left join product_model as pm on pm.ModelId=drd.modelId GROUP BY reportId desc limit $start, $limit");	


}

/*
$qry_prdct = mysql_query("select prd.product,pm.ModelNo from daily_reports_data drd join invoices inv on drd.invoice_id=inv.invoiceId join products prd on prd.productId=drd.productId left join product_model as pm on pm.ModelId=drd.modelId where inv.invoiceNumber='".$_GET['invid']."' order by drd.id desc");
		...
		while( $prd=mysql_fetch_object($qry_prdct) )
		{
			  ...
			  $prd_quantity.="<li>".(int)$prd->quantity."</li>";
			  $modelno.="<li>". $prd->ModelNo."</li>";
		}
			...
			//	paymentTerms = $.trim(paymentTerms);
				...
				{ 
					$.ajax({   
					    ...
					    type: 'POST',
						data:{'ide':ide,'runcard':runcard,'paymentTerms':paymentTerms,'delCommittedDate':delCommittedDate,'delAcceptanceDate':delAcceptanceDate,'vendorDispatch':vendorDispatch,'purchasePoDrop':purchasePoDrop,'purchasePoNumber':purchasePoNumber,'purchasePoDate':purchasePoDate,'purchasePoType':purchasePoType},
						success:function(data){
