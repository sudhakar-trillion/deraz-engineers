<?php include("includes/sa_header.php"); 
 ...
 $numRecords = mysql_query("select employees.firstName from collections 
						...
						left join employees on daily_reports.addedBy = employees.id
						 ...
						 where employees.id = '". $_SESSION['id'] ."'
...
$collections = mysql_query("select employees.firstName, collections.paymentType, collections.amount, collections.paidDate, daily_reports.reportId, daily_reports.contactPerson, daily_reports.invoice from collections 
							left join daily_reports on collections.invoiceId = daily_reports.reportId
							left join employees on daily_reports.addedBy = employees.id
							where employees.id = '". $_SESSION['id'] ."' and collections.paidDate like '%-".$cm."-%'
							order by collections.id desc limit $start, $limit");
			...
			<div class="main-content">
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							<li class="active"> <i class="ace-icon fa fa-home home-icon"></i> Collections </li>
						</ul><!-- /.breadcrumb -->
  ...
  if(mysql_num_rows($totalAmount)>0)
  {
	 ...
	 ?><tr>
                                                        ...
                                                        <?php 
														 ...
														 if($collection['paymentType']==1) { echo 'Proforma'; } 
															 else if($collection['paymentType']==2) { echo 'Through Bank'; } 
															 else if($collection['paymentType']==3) { echo 'Direct Payment'; } 
															 else if($collection['paymentType']==4) { echo 'Against Delivery'; }
														?>
...
<?php  $finalCollected = $finalCollected+$collection['amount'];
 $i++; }
 
 ?> <tr><td colspan="5"></td><td><?php echo $finalCollected; ?></td><td></td></tr>   <?php
...
<![endif]-->
		...
		{
			window.location = 'sa_collections.php?cm='+cm;
		}
