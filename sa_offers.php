...
//   	echo $fromDate = date("Y-m-d",$fromDate);
...
// pagination
 ...
 if( isset($_GET['tmid']) && $_GET['tmid']!='' )
 {
	...
	// get the offer data from daily_reports table
	$numRecords = mysql_query("select daily_reports.reportId, daily_reports.reportDate, daily_reports.contactPerson, daily_reports.enquiryNumber, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.designation, daily_reports.phone, daily_reports.email, daily_reports.clientStatus, daily_reports.leadType, daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.poNo, daily_reports.poValue, daily_reports.paymentType, daily_reports.addedOn, customers.company  from daily_reports 
						  ...
						  where daily_reports.addedBy = '".$Emply_ide."'
...
// search options  where daily_reports.po = '0'
 if( isset($_GET['tmid']) && $_GET['tmid']!='' )
		...
		$whereItems[] = "daily_reports.addedBy = '".$Emply_ide."'";
		}
else
{
  ...
  if(isset($_GET['proposalSearch']))
								...
								if($_GET['fromDate']!='' && $_GET['toDate']!='')

									{
									  ...
									  $whereItems[] = "daily_reports.offerDate >=  '". $fromDate ."'";
								
									  $whereItems[] = "daily_reports.offerDate <=  '". $toDate ."'";
									}
								}
		...
		// get the offer data from daily_reports table											
$result = mysql_query("select daily_reports.reportId, daily_reports.offerNo, daily_reports.reportDate, daily_reports.offerToBeSubmitted, daily_reports.enquiryNumber, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.currentRevisionId, daily_reports.reportDate, daily_reports.contactPerson,  daily_reports.phone, daily_reports.clientStatus, daily_reports.leadType, daily_reports.leadStatus, daily_reports.remarks, daily_reports.futureDate, daily_reports.remarks, daily_reports.po, daily_reports.poNo, daily_reports.reportType, customers.company  from daily_reports 
                          left join customers on daily_reports.company = customers.customerId
  						  left join enquiries on daily_reports.enquiryNumber = enquiries.enquiryNumber
						  $where 
						  GROUP BY daily_reports.reportId desc");
				...
				// get the offer data from daily_reports table						
$result = mysql_query("select daily_reports.reportId, daily_reports.offerNo, daily_reports.reportDate, daily_reports.offerToBeSubmitted, daily_reports.currentRevisionId, daily_reports.enquiryNumber, daily_reports.reportDate, daily_reports.contactPerson,  daily_reports.phone, daily_reports.clientStatus, date_format(enquiries.dateTime,'%d-%m-%Y') enqDate, daily_reports.leadType, daily_reports.leadStatus, daily_reports.remarks, daily_reports.futureDate, daily_reports.remarks, daily_reports.po, daily_reports.poNo, daily_reports.reportType, customers.company  from daily_reports 
                          left join customers on daily_reports.company = customers.customerId
  						  left join enquiries on daily_reports.enquiryNumber = enquiries.enquiryNumber
						  where daily_reports.addedBy = '". $_SESSION['id'] ."' 
				...
				<div class="main-content-inner">
					...
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
                           ...
                           <a href="excel/sa_offers.csv" title="Download"><span class="badge badge-success"><i class="ace-icon fa fa-cloud-download bigger-110 icon-only"></i></span></a>
...
{ $alertMsg = '<div class="alert alert-danger"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Error occured, please try again.</div>'; }
 $branches = mysql_query("select * from branches order by branch");
  ...
  <?php 
   ...
   $list[] = array('S. No', 'Report Date', 'Offer', 'Value', 'Company', 'Person', 'Phone', 'Lead Status' );   
...
if(mysql_num_rows($result)>0)
 ...
 $i = $start+1; 
    while($row = mysql_fetch_array($result))
  {
	 ...
	 $totalAmount = mysql_query("select grandTotal from daily_reports_revision where daily_reports_revision.reportId = '". $row['reportId'] ."' and daily_reports_revision.revision = '". $row['currentRevisionId'] ."'");
  ...
  { $grandTotal = 0; }
	 ...
	 ?><tr style="background:<?php echo $bgcolor; ?>">
											        ...
											        //    $addoffer[] = 'Offer to be generated';
												//	    if((!in_array($row['leadStatus'],$addoffer)) && $row['po']<1)
											?>
                                            ...
                                            <a class="btn btn-success btn-sm" href="sa_view_offer.php?rid=<?php echo $row['reportId'] ?>">
...
<?php 
 ...
 ?> 
  ...
  <?php  } ?>      
                                            ...
                                            <?php                              
$fp = fopen('excel/sa_offers.csv', 'w');
...
{
   window.location = 'sa_offers.php?page='+pid;	
}
  ...
  function confirmDelete(did)
  {
	  ...
	  {
	  window.location = 'sa_offers.php?did='+did;
	  }
  }
