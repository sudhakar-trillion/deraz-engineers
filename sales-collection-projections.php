...
$employees = mysql_query("select employees.id, employees.firstName from employees left join rolls on employees.roll = rolls.roll_id where employees.roll = '4' and employees.id=".$Emply_ide." order by employees.firstName ");
...
// projected collections comes from expected_collections table   
$Projectedqry = mysql_query("select exp_col.amount, invoices.invoiceNumber, invoices.invoiceId, emp.firstName from expected_collections as exp_col join daily_reports as dr on exp_col.reportId=dr.reportId join invoices on invoices.invoiceNumber= dr.invoice join employees as emp on dr.addedBy=emp.id where emp.id=".$employee['id']); 
...
// projected collections comes from collections table   
$Achievedqry = mysql_query("select col.amount,emp.firstName, col.invoiceId from collections as col left join invoices as inv on col.invoiceId=inv.invoiceId left join daily_reports as dr on inv.reportId=dr.reportId left join employees as emp on dr.addedBy=emp.id where emp.id=".$employee['id']." and col.invoiceId=".$Projected->invoiceId); 
 
 
if(mysql_num_rows($Achievedqry)>0)
{
 ...
 ?><tr>
                                                       ...
                                                       <td><?PHP echo $Projected->invoiceNumber; ?></td>
					                                                         
													   <td><?PHP if($Projected->firstName!=''){ echo $Projected->amount; } else echo "0";?> </td>
                                                         <td><?PHP if($amnt_acheived!=''){ echo $amnt_acheived; } else echo "0";?> </td>
