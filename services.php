...
// data comes from services table
 ...
 else
 {
    ...
    if(isset($_GET['search']))
							...
							/* category search based on categories.id
							   by type of service based on services.typeOfService
							    */		
...
$numRecords = mysql_query("select services.complaintNumber, services.serialNumber, services.serviceId, services.typeOfService,services.category, services.serviceId, services.customerName, services.product, services.modelNo, services.date, services.inwardDetails, services.engineer, services.quantity from services  $where order by serviceId desc");
...
// data comes from services table												
$services = mysql_query("select services.brand, services.serviceId, services.serialNumber, services.complaintNumber, services.typeOfService, services.category, services.customerName, services.product, services.modelNo, services.date, services.inwardDetails, services.engineer, services.quantity from services 
$where order by serviceId desc");												

#echo "select services.brand, services.serviceId, services.serialNumber, services.complaintNumber, services.typeOfService, services.category, services.customerName, services.product, services.modelNo, services.date, services.inwardDetails, services.engineer, services.quantity from services $where order by serviceId desc"; 
}

else 
{
	$numRecords = mysql_query("select services.complaintNumber, services.serialNumber, services.serviceId, services.typeOfService,services.category, services.serviceId, services.customerName, services.product, services.modelNo, services.date, services.inwardDetails, services.engineer, services.quantity from services 
...
// data comes from services table	
$services = mysql_query("select services.inwardDetails, services.brand, services.serialNumber, services.typeOfService, services.complaintNumber, services.category, services.serviceId, services.customerName, services.product, services.modelNo, services.date, services.engineer, services.quantity from services  $where order by serviceId desc limit $start, $limit");
}
...
?>
  ...
  <div class="form-group col-sm-2">
   <span>Engineer</span>
    <input type="text" class="form-control input-sm" id="engineer" name="engineer" placeholder="Engineer Name" <?php if(isset($_GET['engineer'])) { ?> value="<?php echo $_GET['engineer']; ?>" <?php } ?> />
  </div> 
...
{ $alertMsg = '<div class="alert alert-info"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Service has been deleted!</div>'; }
  ...
  <?php 
   $list[] = array('S. No', 'Date', 'Customer', 'DC no', 'Gate Pass', 'Category', 'Product', 'Model No', 'Quantity', 'Engineer', 'Status','Remarks' );
  ...
  <td>
														...
														<td>
															<?php echo $service['complaintNumber']; ?>
														</td>                                                        
                                                         <td>
         ...
         <span class="label label-sm label-danger"><?php echo $serviceStatus;   ?></span>
                          ...
                          <?php if(isset($_SESSION['superAdmin']) || isset($_SESSION['logistics'])) {  ?>  
...
$i++;   } } else { ?> <tr><td colspan="13">No Data found.</td></tr> <?php } 
...
$(function()
 {
	...
	$("#engineer").autocomplete({
		 source: "Requestdispatcher.php?func=getengnieers",
		// type:'POST'
	});		
