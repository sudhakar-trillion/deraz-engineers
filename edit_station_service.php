...
mysql_query("update services_station  set `date` = '$date', `customerName` = '$customerName', `product` =  '$product', `modelNo` = '$modelNo', `equipmentNo` = '$equipmentNo', `quantity` = '$quantity', `complaint` = '$natureComplaint', `engineer` = '$engineer' where serviceId = '". $_GET['sid'] ."'");
	
	//  mysql_query("insert into service_status (`serviceId`, `status`, `updatedOn`) values ('". $_GET['sid'] ."', '$status', NOW())");   
	  header("location: edit_station_service.php?sid=".$_GET['sid']."&add=1");	
}		
			
	


  $services = mysql_query("select * from services_station where serviceId = '". $_GET['sid'] ."'");

if(mysql_num_rows($services)<1)
{
  header("location: station_services.php"); 								
}
