...
// pagination
 ...
 { 
	...
	// get the data from enquiries,enquiry_assign,enquiry_products table
		$numRecords = mysql_query("select enquiries.enquiryId, daily_reports.leadStatus, enquiries.name, enquiries.enquiryNumber, enquiry_assign.dateandtime, enquiries.enqDate, customers.company, enquiries.email, enquiries.source, enquiries.phone, enquiry_assign.assignedTo from enquiries
...
where enquiry_assign.assignedTo = '".$EmployeeName."' GROUP BY enquiries.enquiryId desc");
...
left join customers on enquiries.company = customers.customerId
left join daily_reports on enquiries.enquiryNumber = daily_reports.enquiryNumber
where enquiry_assign.assignedTo = '". $_SESSION['firstName'] ."' GROUP BY enquiries.enquiryId desc");
 }
...
if(isset($_GET['reportSearch']))
{
									...
									if(isset($_GET['tmid']) && $_GET['tmid']!='')
									{
										...
										$where = "where enquiry_assign.assignedTo = '".$_SESSION['firstName']."' and ";
									}
									  ...
									  $whereItems[] = "enquiries.dateTime <=  '". $toDate ."'";
									   ...
									   {  
										...
										$_GET['customer']='';
												...
												{  
										$where = "where enquiry_assign.assignedTo = '".$_SESSION['firstName']."' ";
												}
//$Emply_ide,$EmployeeName

 if( isset($_GET['tmid']) && $_GET['tmid']!='' )
{
	$query = "select enquiries.name, daily_reports.leadStatus, enquiries.enquiryId, enquiries.enquiryNumber, enquiry_assign.dateandtime, enquiries.enqDate, customers.company, enquiries.email, enquiries.source, enquiries.phone,  enquiry_assign.assignedTo from enquiries
left join enquiry_assign on enquiries.enquiryId= enquiry_assign.enquiryId 
left join customers on enquiries.company = customers.customerId
left join daily_reports on enquiries.enquiryNumber = daily_reports.enquiryNumber
join employees as empl on empl.id='".$Emply_ide."' $where GROUP BY enquiries.enquiryId desc";
}
else
{ 	// get the data from enquiries,enquiry_assign,enquiry_products table
  $query = "select enquiries.name, daily_reports.leadStatus, enquiries.enquiryId, enquiries.enquiryNumber, enquiry_assign.dateandtime, enquiries.enqDate, customers.company, enquiries.email, enquiries.source, enquiries.phone,  enquiry_assign.assignedTo from enquiries
...
left join customers on enquiries.company = customers.customerId
left join daily_reports on enquiries.enquiryNumber = daily_reports.enquiryNumber
join employees as empl on empl.id='". $_SESSION['id'] ."' $where GROUP BY enquiries.enquiryId desc";
}
//echo $query; exit; 
...
else
{
 if( isset($_GET['tmid']) && $_GET['tmid']!='' )
	...
	left join customers on enquiries.company = customers.customerId
	left join daily_reports on enquiries.enquiryNumber = daily_reports.enquiryNumber
	join employees as empl on empl.id='".$Emply_ide."' where enquiry_assign.assignedTo = '".$EmployeeName."' GROUP BY enquiries.enquiryId desc";
	...
	left join customers on enquiries.company = customers.customerId
	left join daily_reports on enquiries.enquiryNumber = daily_reports.enquiryNumber
	join employees as empl on empl.id='". $_SESSION['id'] ."' where enquiry_assign.assignedTo = '". $_SESSION['firstName'] ."' GROUP BY enquiries.enquiryId desc";
	}
}
...
{ echo '<div class="alert alert-danger"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Error occured, please try again.</div>'; }
  ...
  while($proposal = mysql_fetch_array($proposals))
  {
	 ?><tr>
                                                        ...
                                                        <td>
															<?php $date = explode(' ',$proposal['enqDate']);
