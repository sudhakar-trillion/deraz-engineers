<?php include("includes/sa_header.php"); 
		...
		if(isset($_POST['addproducts']))
		{
		   ...
		   mysql_query("update daily_reports set `total` =  '". $_POST['total'] ."' where reportId = '". $_GET['pid'] ."' and addedBy = '". $_SESSION['id'] ."'");		
		   
		   	for($i=0;$i<$count;$i++)
			{
			   echo "insert into daily_reports_data (`reportId`, `categoryId`, `brandId`, `productId`, `price`, `quantity`, `amount`) values ('". $_GET['pid'] ."', '". $_POST['categories'][$i] ."', '". $_POST['brands'][$i] ."', '". $_POST['product'][$i] ."', '". $_POST['quantity'][$i] ."', '". $_POST['price'][$i] ."', '". $_POST['amount'][$i] ."')"; 
			   echo '<br />';
			   mysql_query("insert into daily_reports_data (`reportId`, `categoryId`, `brandId`, `productId`, `price`, `quantity`, `amount`) values ('". $_GET['pid'] ."', '". $_POST['categories'][$i] ."', '". $_POST['brands'][$i] ."', '". $_POST['product'][$i] ."', '". $_POST['quantity'][$i] ."', '". $_POST['price'][$i] ."', '". $_POST['amount'][$i] ."')");	
			}
        ...
        $lastId = mysql_insert_id();
			...
			if($lastId>0)
			{
			  header("location: sa_add_products_daily_report.php?add=1");	
			}
			else
			{
			  header("location: sa_add_products_daily_report.php?error=1");	
			}
...
$poDetails = mysql_query("select date, poNo from daily_reports where reportId = '". $_GET['pid'] ."'");
$poDetails = mysql_fetch_array($poDetails);

$date = explode('-',$poDetails['date']);
								...
								<div class="row">
									<div class="col-xs-12">
										...
										<div class="table-header">
											<?php echo 'Date : '.$date.', Po No : '.$poDetails['poNo'];  ?>
										</div>
...
{ $alertMsg = '<div class="alert alert-danger"><button class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>Authorization required.</div>'; }
  ...
  if(isset($alertMsg)) { echo $alertMsg; }
                                ...
                                <div class="col-sm-2">
                                <span>Category</span>
                                <select id="category1" name="categories[]" class="col-xs-10 col-sm-12" onChange="getProducts('1')">
                                ...
                                <select id="brand1" name="brands[]" class="col-xs-10 col-sm-12" onChange="getProducts('1')">
                                 ...
                                 <div class="col-sm-2">
                                <span>Product</span>
                                <select name="product[]" id="product1" class="col-xs-10 col-sm-12" onChange="getProductPrice('1',this.value)">
                                ...
                                <div id="productdetails1"> 
                                 <div class="col-sm-2">
                                <span>quantity</span>
                                <input type="text" name="quantity[]" id="quantity1" value="" onkeyup="getPrice('1')"  />
                                </div> 
                                 <div class="col-sm-2">
                                <span>Price</span>
                                <input type="text" name="price[]" id="price1" value=""  onkeyup="getPrice('1')"   />
                                </div> 
                                 <div class="col-sm-2">
                                <span>Amount</span>
                                    ...
                                    <div class="clearfix form-actions">
										<div class="col-md-offset-5 col-md-8">
                                          ...
                                          <div class="col-sm-offset-2 col-sm-1">
                                            <button class="btn btn-sm btn-info" type="submit" name="addproducts">
                                            ...
                                            <button class="btn btn-sm btn-danger" type="button" id="removeRow" onclick="removeFields(this.value)" value="1">
                                             ...
                                             <div class="col-sm-4">
                                            <span>Total</span>
                                            <input type="text" id="total" name="total"  size="12" value="0" />
                                            </div>
										</div>
									</div>
...
<![endif]-->
 ...
 function getPrice(rid)
 {
	...
	if(quantity==0)
	{  
		document.getElementById("quantity"+rid).focus();
	}
	else if(price==0)
	{  
		document.getElementById("price"+rid).focus();
	}
	else if(quantity>0 && price>0)
	{
	    document.getElementById("amount"+rid).value = quantity*price;
		
	}
	 
	 
	 
	 var numRows = document.getElementById('addRow').value;
