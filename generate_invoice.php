<?php  include("includes/header.php"); 
  ...
  // generate Proforma Invoice if the status is pending proforma invoice.
		if(isset($_POST['generateProformaInvoice']))
		{
			$status = 'Proforma generated';
		  
		
		 mysql_query("update daily_reports set leadStatus = '". $status ."', inv = '1', `paymentStatus` = 'open' where reportId = '". $_GET['pid'] ."'");
		
		
		mysql_query("insert into invoices (`reportId`, `subTotal`, `discount`, `total`, `dutyPercentage`, `dutyAmount`, `pfPercentage`, `pfAmount`, `grandTotal`, `proInvoiceNumber`, `proInvoiceDateTime`) values('". $_GET['pid'] ."',  '". $_POST['subTotal'] ."', '". $_POST['discount'] ."', '". $_POST['total'] ."', '". $_POST['dutyPercentage'] ."', '". $_POST['dutyAmount'] ."', '". $_POST['pfPercentage'] ."', '". $_POST['pfAmount'] ."', '". $_POST['grandTotal'] ."', '". $_POST['proformaInvoice'] ."', NOW())");   
  ...
  // generate Invoice if the status is pending invoice
		if(isset($_POST['generateInvoice']))
		{
			
			  $status = 'Invoice generated';
			  
			  $invoiceDateTime = $_POST['invoiceDateTime'];
				$invoiceDateTime1 = date_create($invoiceDateTime);
				$invoiceDateTime1 = date_format($invoiceDateTime1,'Y-m-d');
			
$lastId =	mysql_query("update invoices set invoiceNumber = '". $_POST['invoice'] ."', invoiceDateTime = '".$invoiceDateTime1."',  invoiceStatus = '". $status ."' where invoiceId = '". $_GET['iid'] ."'");
		   
		   if($lastId>0)
		   {
			 $result = mysql_query("select reportId from invoices where invoiceId = '". $_GET['iid'] ."'");
			 $result = mysql_fetch_array($result);  
			   
		 
		   mysql_query("update daily_reports set leadStatus = '". $status ."', inv = '2', invoice = '". $_POST['invoice'] ."', invoiceDateTime = NOW() where reportId = '". $result['reportId'] ."'");
		   header("location: generate_invoice.php?iid=".$_GET['iid']."&success=1");     
		   }
		   else
		   {
			   header("location: generate_invoice.php?iid=".$_GET['iid']."&error=0");     
		   }
		   
		}		


$proposals = mysql_query("select daily_reports.reportId,daily_reports.offerNo,date_format(enquiries.dateTime,'%d-%m-%Y') as enqDate, date_format(daily_reports.offerDate,'%d-%m-%Y') as offerDate, date_format(daily_reports.proformaInvoiceDateTime,'%d-%m-%Y') as proformaInvoiceDateTime, daily_reports.currentRevisionId, daily_reports.enquiryNumber, daily_reports.reportDate, daily_reports.company, daily_reports.contactPerson, daily_reports.designation, daily_reports.phone, daily_reports.email, daily_reports.clientStatus, daily_reports.leadType, daily_reports.leadStatus, daily_reports.futureDate, daily_reports.remarks, daily_reports.poNo, daily_reports.poDateTime, daily_reports.paymentType, daily_reports.inv, daily_reports.invoice, daily_reports.invoiceDateTime, customers.company, employees.firstName, invoices.invoiceNumber from invoices
                                      ...
                                      <table id="sample-table-1" class="table table-striped table-bordered table-hover">
											<tbody>
											  ...
											  <tr>
												...
												$proInvoices = mysql_query("select proInvoiceNumber from invoices where invoiceId = '". $_GET['iid'] ."'");
												...
												} else if(count($pro_inv)>1)
												{
                                                  ...
                                                  <tr>
                                                   ...
                                                   <td>
                                                   	<?PHP echo $proposal['proformaInvoiceDateTime']; ?>													 
												  </td>
												</tr>
             ...
             <tr><td colspan="5"></td><td><?php echo $item['taxSystem'].' '.$item['taxPercentage'].'%'; ?></td><td><?php echo $item['taxAmount']; ?></td></tr>                                      
								...
								$i++;	 }
                                    ...
                                    <input type="hidden" id="crid" value="<?php echo $revision['revisionId']; ?>" />
                                     ...
                                     <tr><td colspan="5"></td><td>Pro Invoice</td><td><?php echo $pro_inv; ?></td></tr>  
                                    
                                      <?php if($proposal['inv']==1) { ?>
                                     <tr><td colspan="5"></td><td>	<label for="invoice"> Invoice No. </label>
                                     
                                   
                                     </td>
                                         <td><input type="text" id="invoice" name="invoice" placeholder="Invoice No."></td></tr> 
...
<input type="text" name="invoiceDateTime" id="invoiceDateTime" class="date-picker" placeholder="Invoice Date" /></td>
                                   </tr>
                                       
                                         
         <tr><td colspan="5"></td><td></td>
                                         <td>
                                         <button class="btn btn-sm btn-info" type="submit" name="generateInvoice">
                                      ...
                                      <?php } else { ?>
                                   <tr><td colspan="5"></td><td>Invoice Number</td><td><?php echo $proposal['invoiceNumber']; ?></td></tr>       
                                      <?php } ?>
