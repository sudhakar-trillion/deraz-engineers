...
$limit = 10;
 ...
 $currentMonth = date('m');
$numRecords = mysql_query("select DISTINCT employees.id, employees.firstName, daily_reports.enquiryNumber, customers.company from invoices 
...
/*echo "select DISTINCT employees.id, employees.firstName from invoices 
						  left join daily_reports on invoices.reportId = daily_reports.reportId
						  left join employees on employees.id = daily_reports.addedBy
						where employees.roll = '4' and  daily_reports.inv > '0' and invoices.paymentStatus = 'open' and  invoices.proInvoiceDateTime like '%-". $currentMonth ."-%'  order by employees.firstName"; exit; */
...
$employees = mysql_query("select DISTINCT employees.id, customers.company,branches.branch, daily_reports.enquiryNumber, employees.firstName from invoices 
						  left join daily_reports on invoices.reportId = daily_reports.reportId
						  left join employees on employees.id = daily_reports.addedBy
...
else								
{
	 ...
	 $whereItems[] = "invoices.proInvoiceDateTime like '%-". $currentMonth ."-%'";
						...
						$employees = mysql_query("select DISTINCT employees.id, employees.firstName, daily_reports.enquiryNumber, customers.company, branches.branch from invoices 
						  left join daily_reports on invoices.reportId = daily_reports.reportId
						  left join employees on employees.id = daily_reports.addedBy 
						  left join branches on employees.branch=branches.branchid
  						  left join customers on daily_reports.company = customers.customerId
...
<div class="space"></div>
 ...
 $list[] = array('S. No', 'Employee', 'Branch Name','Company Name', 'Invoice Date', 'Invoice Number', 'Total', 'Outstanding' );
  ...
  while($employee = mysql_fetch_array($employees))
  {
	 ?>
      <tr>
                                                      ...
                                                      <td>
													 <?php if($employee['enquiryNumber']!='')
													 {
													  echo $employee['enquiryNumber']; }
													  else
													  {
														  echo '--';
													  }
...
$invo = mysql_query("select distinct(invoices.invoiceNumber),invoices.invoiceId, invoices.invoiceDateTime from invoices left join daily_reports on invoices.reportId  = daily_reports.reportId left join employees on employees.id=daily_reports.addedBy  where daily_reports.addedBy = '". $employee['id'] ."'");


													
													
													while($invc = mysql_fetch_array($invo))
													{
														$invqq = $invc['invoiceId'];
														if(trim($invc['invoiceNumber']!=''))
														{
															$invoice=$invc['invoiceNumber'];
														}
														
														$invDate1 = date_create($invc['invoiceDateTime']);
														$invDate = date_format($invDate1,'d-m-Y');
														$enddated = $invc['enqdt'];
													}
...
//date_format(enquiries.dateTime,'%d-%m-%Y') as enqdt

$qry_prdct = mysql_query("select drd.price,drd.quantity, date_format(enquiries.dateTime,'%d-%m-%Y') as enqdt, prd.product,inv.invoiceNumber from daily_reports_data drd join invoices inv on drd.invoice_id=inv.invoiceId join products prd on prd.productId=drd.productId left join daily_reports on daily_reports.reportId=inv.reportId left join enquiries on enquiries.enquiryNumber = daily_reports.enquiryNumber where drd.invoice_id='".$invqq."' order by drd.id desc limit 1");

//echo "select drd.price,drd.quantity, date_format(enquiries.dateTime,'%d-%m-%Y') as enqdt, prd.product,inv.invoiceNumber from daily_reports_data drd join invoices inv on drd.invoice_id=inv.invoiceId join products prd on prd.productId=drd.productId left join daily_reports on daily_reports.reportId=inv.reportId left join enquiries on enquiries.enquiryNumber = daily_reports.enquiryNumber where drd.invoice_id='".$invqq."' order by drd.id desc limit 1"; exit; 

//echo "select drd.price,drd.quantity, prd.product,inv.invoiceNumber from daily_reports_data drd join invoices inv on drd.invoice_id=inv.invoiceId join products prd on prd.productId=drd.productId where drd.invoice_id='".$_GET['invoiceId']."' order by drd.id desc limit 1"; exit; 
	  $prd_name='';
	  $prd_quantity=0;
	  $enqdated='';
	  if(mysql_num_rows($qry_prdct)>0)
	  {
		while($prd = mysql_fetch_object($qry_prdct))  
		{
			  ...
			  $enqdated=$prd->enqdt;
													  ...
													  ?>
                                                       <td><?PHP if($enqdated!=''){ echo $enqdated; } else echo "--"; //enqdt; ?> </td>
   ...
   <tr><td><?php echo $sno; ?></td>
    ...
    <tr><td colspan="7"></td><td><?php echo $totalPending; ?></td></tr>
    </tbody></table>
                           ...
                           <a href="<?php echo '#modal-table'.$i; ?>" role="button" class="green" data-toggle="modal"><?php echo $totalPending;	?></a>
                                                  ...
                                                  <td> <a href="outstandings-view.php?empid=<?PHP echo $employee['id']; ?>&invid=<?PHP echo $invoice;?>" class="btn btn-warning btn-sm"><i class="ace-icon fa fa-eye icon-only"></i></a> </td>
                                                       </tr>
                                                       <?php $i++; }
  }
 
 ?> <tr><td colspan="9"></td><td><?php echo $grandTotalPending; ?></td></tr><?php
